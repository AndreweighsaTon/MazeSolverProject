...
Getting Started
You will need to install Java 11 if you have not already. If you are on Windows, you will also need Git Bash.
...
Mazes come in all sorts of shapes and sizes (circle mazes, hedge mazes...)
...
First, you'll need to fill out the constructor. Documentation for the Java Random object will help with instantiating gen. You can pass in seed to gen to make sure that graphs with the same seed will always come out the same (since gen outputs a deterministic set of pseudorandom numbers if given the same seed).
...
Wikipedia has a description of several maze generation algorithms if you'd like to base your solution off of one (make sure to cite your sources though!)
...
All tiles must be connected. (There shouldn't be a square in the maze that can't be reached from the start.)
Tiles can only be directly connected to its neighbors (see the comment on top of getNeighbors() for more details).
Mazes created with the same seed, width, and height inputs should have the same layout (i.e. m1.equals(m2) should return true).
Mazes created with different seeds should be different. (Don't just return a maze with no walls every single time!!)
Writing Tests
...
After completing this section, you should be able to click the "Solve" button and have an output that looks something like this: Solved maze
...
What algorithm did you choose for the maze generator? How does it work (in simple terms)?
How did you create your maze solver?
What was the biggest/best/worst/most fun challenge or bug that you solved while working on this project?
